...
export const MovieDialogue = forwardRef<MovieDialogueRef, MovieDialogueProps>(({ who, text, onNext }, ref) => {
      ...
      {characterName ? (
        <View className="flex-row items-center gap-2 mb-3">
          <View className="px-3 py-1 bg-[#2B1B14] rounded-lg border border-[#FF8517]">
            <ThemedText className="text-[#FF8517] font-bold text-[13px] tracking-widest">{characterName.toUpperCase()}</ThemedText>
          </View>
          {role ? <ThemedText className="text-[#A3A3AF] text-[13px] italic">{role}</ThemedText> : null}
        </View>
      ) : null}

      {/* Dialogue Text */}
      <ThemedText className="text-white text-[17px] leading-7 flex-1">
        {displayedText}
      </ThemedText>
