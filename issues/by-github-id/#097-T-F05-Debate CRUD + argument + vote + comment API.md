...
- Endpoint base: `/api/v1/debates`. Nếu task cần nhiều action, dùng REST sub-route rõ nghĩa dưới base này.
...
- Các trường output tối thiểu phải đủ để frontend render trực tiếp các AC: FOR/AGAINST/NEUTRAL; unique vote.
...
- Issue này phải được triển khai trên branch riêng và PR phải link trực tiếp tới issue #97.
- Nếu phát hiện dependency chưa sẵn sàng, PR phải ghi rõ mock/contract tạm và điều kiện để chuyển sang integration thật.

## 5. Hành vi người dùng hoặc API cần đạt

- Client gọi endpoint dưới `/api/v1/debates` với input hợp lệ thì nhận response thành công đủ dữ liệu để render hoặc lưu state.
...
- Output cần expose field trực tiếp phục vụ AC: FOR/AGAINST/NEUTRAL, unique vote.

## 6. Acceptance Criteria chi tiết

- [ ] FOR/AGAINST/NEUTRAL: có bằng chứng kiểm chứng rõ ràng trong PR.
- [ ] unique vote: có bằng chứng kiểm chứng rõ ràng trong PR.
...
> Added by BMAD Advanced Elicitation on 2026-05-31. This section defines the concrete product output expected from issue #97 / `T-F05`, beyond implementation process notes.
...
- Một hoặc nhiều endpoint dưới `/api/v1/debates` hoạt động với request hợp lệ và trả response chuẩn `{ success, data, meta? }`.
...
- Client gọi `/api/v1/debates`, nhận HTTP 2xx với data đủ field để consumer render/lưu state.
...
- PR description must link issue #97 and mention `T-F05`.
- If the final behavior differs from this contract, update the issue and local docs in the same PR.

## Status Log

- 2026-06-01: BMAD sprint-status sync checked GitHub issue #97 for `T-F05`. Current source-of-truth status: **OPEN**. Local log: `issues/by-github-id/#097-T-F05-Debate CRUD + argument + vote + comment API.md`.
