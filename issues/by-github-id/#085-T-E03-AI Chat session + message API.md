...
- Issue: [#85](https://github.com/khovan123/philo-mind/issues/85)
- State: open
...
- Updated at: 2026-06-01T05:30:46Z
...
- 2026-05-31: Synced from GitHub issue state. This local file exists so the plan has an auditable log for issue #85 / `T-E03`.
...
| GitHub issue   | #85                                                               |
| Task ID        | T-E03                                                             |
| Title          | AI Chat session + message API                                     |
| State          | open                                                              |
| Local log path | `issues/by-github-id/#085-T-E03-AI Chat session + message API.md` |
...
- Endpoint base: `/api/v1/ai/chat`. Nếu task cần nhiều action, dùng REST sub-route rõ nghĩa dưới base này.
...
- Các trường output tối thiểu phải đủ để frontend render trực tiếp các AC: Start/list/get sessions; send message; 20-msg context.
...
- Dependency trước khi tích hợp: `T-E02`.
...
- Issue này phải được triển khai trên branch riêng và PR phải link trực tiếp tới issue #85.
- Nếu phát hiện dependency chưa sẵn sàng, PR phải ghi rõ mock/contract tạm và điều kiện để chuyển sang integration thật.

## 5. Hành vi người dùng hoặc API cần đạt

- Client gọi endpoint dưới `/api/v1/ai/chat` với input hợp lệ thì nhận response thành công đủ dữ liệu để render hoặc lưu state.
...
- Output cần expose field trực tiếp phục vụ AC: Start/list/get sessions, send message, 20-msg context.

## 6. Acceptance Criteria chi tiết

- [ ] Start/list/get sessions: có bằng chứng kiểm chứng rõ ràng trong PR.
- [ ] send message: có bằng chứng kiểm chứng rõ ràng trong PR.
- [ ] 20-msg context: có bằng chứng kiểm chứng rõ ràng trong PR.
...
> Added by BMAD Advanced Elicitation on 2026-05-31. This section defines the concrete product output expected from issue #85 / `T-E03`, beyond implementation process notes.
...
- Một hoặc nhiều endpoint dưới `/api/v1/ai/chat` hoạt động với request hợp lệ và trả response chuẩn `{ success, data, meta? }`.
...
- Client gọi `/api/v1/ai/chat`, nhận HTTP 2xx với data đủ field để consumer render/lưu state.
...
- PR description must link issue #85 and mention `T-E03`.
...
- 2026-05-31: BMAD sprint-status sync checked GitHub issue #85 for `T-E03`. Current source-of-truth status: **OPEN**. Local log: `issues/by-github-id/#085-T-E03-AI Chat session + message API.md`.
