# T-B13: Quiz gameplay screen (questions + timer + result)

## GitHub Link

- Issue: [#51](https://github.com/khovan123/philo-mind/issues/51)
- State: done
...
- Updated at: 2026-06-03T11:11:00Z
- Closed at: 2026-06-03T11:11:00Z
...
- 2026-05-31: Synced from GitHub issue state. This local file exists so the plan has an auditable log for issue #51 / `T-B13`.
...
| GitHub issue   | #51                                                                                   |
| Task ID        | T-B13                                                                                 |
| Title          | Quiz gameplay screen (questions + timer + result)                                     |
| State          | done                                                                                  |
| Local log path | `issues/by-github-id/#051-T-B13-Quiz gameplay screen (questions + timer + result).md` |
...
- Screen/route đề xuất: `/quiz/[lessonId]` theo Expo Router.
...
- UI phải thể hiện trực tiếp các AC: Color feedback; celebration animation.
...
- User mở màn hình qua route `/quiz/[lessonId]`; các CTA phải điều hướng tới màn hình chi tiết hoặc bước kế tiếp có data id/session id.
- Nếu backend chưa sẵn sàng, tạo adapter/mock cùng shape với API thật để khi issue dependency merge chỉ thay data source.
- Issue này phải được triển khai trên branch riêng và PR phải link trực tiếp tới issue #51.
- Nếu phát hiện dependency chưa sẵn sàng, PR phải ghi rõ mock/contract tạm và điều kiện để chuyển sang integration thật.

## 5. Hành vi người dùng hoặc API cần đạt

- Người dùng có thể mở màn hình tại `/quiz/[lessonId]` từ tab/card/link liên quan.
...
- [ ] Color feedback: có bằng chứng kiểm chứng rõ ràng trong PR.
- [ ] celebration animation: có bằng chứng kiểm chứng rõ ràng trong PR.

## 7. Checklist triển khai

- [ ] Khảo sát screen/component dùng chung hiện có và tái sử dụng design tokens của repo.
- [ ] Triển khai đầy đủ UI flow **Quiz gameplay screen (questions + timer + result)** gồm loading, empty, error và interaction state phù hợp.
...
- [ ] Đối chiếu kết quả với yêu cầu cốt lõi: Color feedback, celebration animation.
...
> Added by BMAD Advanced Elicitation on 2026-05-31. This section defines the concrete product output expected from issue #51 / `T-B13`, beyond implementation process notes.
...
- Một màn hình/flow tại `/quiz/[lessonId]` render được trạng thái loading, empty, error và success.
...
- User thao tác trên `/quiz/[lessonId]`, thấy dữ liệu/render đúng, CTA chính chuyển sang bước kế tiếp hoặc cập nhật UI ngay.
...
- PR description must link issue #51 and mention `T-B13`.
