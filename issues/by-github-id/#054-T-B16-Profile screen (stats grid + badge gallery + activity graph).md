...
- Issue: [#54](https://github.com/khovan123/philo-mind/issues/54)
- State: done
...
- Updated at: 2026-06-03T11:11:07Z
- Closed at: 2026-06-03T11:11:06Z
...
- Screen/route đề xuất: `/(tabs)/profile` theo Expo Router.
...
- UI phải thể hiện trực tiếp các AC: GitHub-style heatmap; settings.
...
- User mở màn hình qua route `/(tabs)/profile`; các CTA phải điều hướng tới màn hình chi tiết hoặc bước kế tiếp có data id/session id.
- Nếu backend chưa sẵn sàng, tạo adapter/mock cùng shape với API thật để khi issue dependency merge chỉ thay data source.
- Issue này phải được triển khai trên branch riêng và PR phải link trực tiếp tới issue #54.
- Nếu phát hiện dependency chưa sẵn sàng, PR phải ghi rõ mock/contract tạm và điều kiện để chuyển sang integration thật.

## 5. Hành vi người dùng hoặc API cần đạt

- Người dùng có thể mở màn hình tại `/(tabs)/profile` từ tab/card/link liên quan.
...
- [ ] GitHub-style heatmap: có bằng chứng kiểm chứng rõ ràng trong PR.
- [ ] settings: có bằng chứng kiểm chứng rõ ràng trong PR.

## 7. Checklist triển khai

- [ ] Khảo sát screen/component dùng chung hiện có và tái sử dụng design tokens của repo.
- [ ] Triển khai đầy đủ UI flow **Profile screen (stats grid + badge gallery + activity graph)** gồm loading, empty, error và interaction state phù hợp.
...
- [ ] Đối chiếu kết quả với yêu cầu cốt lõi: GitHub-style heatmap, settings.
...
> Added by BMAD Advanced Elicitation on 2026-05-31. This section defines the concrete product output expected from issue #54 / `T-B16`, beyond implementation process notes.
...
- Một màn hình/flow tại `/(tabs)/profile` render được trạng thái loading, empty, error và success.
...
- User thao tác trên `/(tabs)/profile`, thấy dữ liệu/render đúng, CTA chính chuyển sang bước kế tiếp hoặc cập nhật UI ngay.
...
- PR description must link issue #54 and mention `T-B16`.
