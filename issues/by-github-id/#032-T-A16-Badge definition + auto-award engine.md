...
- Endpoint base: `/api/v1/badges`. Nếu task cần nhiều action, dùng REST sub-route rõ nghĩa dưới base này.
...
- Các trường output tối thiểu phải đủ để frontend render trực tiếp các AC: 10 badge conditions; triggered on activity.
...
- Dependency trước khi tích hợp: `T-A09`.
...
- Issue này phải được triển khai trên branch riêng và PR phải link trực tiếp tới issue #32.
- Nếu phát hiện dependency chưa sẵn sàng, PR phải ghi rõ mock/contract tạm và điều kiện để chuyển sang integration thật.

## 5. Hành vi người dùng hoặc API cần đạt

- Client gọi endpoint dưới `/api/v1/badges` với input hợp lệ thì nhận response thành công đủ dữ liệu để render hoặc lưu state.
...
- Output cần expose field trực tiếp phục vụ AC: 10 badge conditions, triggered on activity.

## 6. Acceptance Criteria chi tiết

- [ ] 10 badge conditions: có bằng chứng kiểm chứng rõ ràng trong PR.
- [ ] triggered on activity: có bằng chứng kiểm chứng rõ ràng trong PR.
...
> Added by BMAD Advanced Elicitation on 2026-05-31. This section defines the concrete product output expected from issue #32 / `T-A16`, beyond implementation process notes.
...
- Một hoặc nhiều endpoint dưới `/api/v1/badges` hoạt động với request hợp lệ và trả response chuẩn `{ success, data, meta? }`.
...
- Client gọi `/api/v1/badges`, nhận HTTP 2xx với data đủ field để consumer render/lưu state.
