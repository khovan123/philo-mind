...
- Type: backend
- Updated at: 2026-05-31T15:53:05Z
...
> Added by BMAD Advanced Elicitation on 2026-05-31. This section defines the concrete product output expected from issue #28 / `T-A12`, beyond implementation process notes.

### User-facing outcome

Đầu ra là một phần tính năng hoàn chỉnh cho Critical Question API (list/random/admin CRUD), có hành vi quan sát được qua UI, API hoặc test.
...
- Một hoặc nhiều endpoint dưới `/api/v1/critical-questions` hoạt động với request hợp lệ và trả response chuẩn `{ success, data, meta? }`.
...
- Client gọi `/api/v1/critical-questions`, nhận HTTP 2xx với data đủ field để consumer render/lưu state.
...
- PR description must link issue #28 and mention `T-A12`.
- If the final behavior differs from this contract, update the issue and local docs in the same PR.

## Status Log

- 2026-06-01: BMAD sprint-status sync checked GitHub issue #28 for `T-A12`. Current source-of-truth status: **DONE**. Closed at: 2026-05-31T10:33:41Z. Local log: `issues/by-github-id/#028-T-A12-Critical Question API (list-random-admin CRUD).md`.
