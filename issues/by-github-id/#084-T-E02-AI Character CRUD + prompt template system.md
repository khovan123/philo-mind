# T-E02: AI Character CRUD + prompt template system
...
- 2026-05-31: Synced from GitHub issue state. This local file exists so the plan has an auditable log for issue #84 / `T-E02`.
...
- Endpoint base: `/api/v1/ai/characters`. Nếu task cần nhiều action, dùng REST sub-route rõ nghĩa dưới base này.
...
- Các trường output tối thiểu phải đủ để frontend render trực tiếp các AC: System prompt builder; admin CRUD.
...
- Dependency trước khi tích hợp: `T-E01`, `T-A04`.
...
- Issue này phải được triển khai trên branch riêng và PR phải link trực tiếp tới issue #84.
- Nếu phát hiện dependency chưa sẵn sàng, PR phải ghi rõ mock/contract tạm và điều kiện để chuyển sang integration thật.

## 5. Hành vi người dùng hoặc API cần đạt

- Client gọi endpoint dưới `/api/v1/ai/characters` với input hợp lệ thì nhận response thành công đủ dữ liệu để render hoặc lưu state.
...
- Output cần expose field trực tiếp phục vụ AC: System prompt builder, admin CRUD.

## 6. Acceptance Criteria chi tiết

- [ ] System prompt builder: có bằng chứng kiểm chứng rõ ràng trong PR.
- [ ] admin CRUD: có bằng chứng kiểm chứng rõ ràng trong PR.

## 7. Checklist triển khai

- [ ] Khảo sát module hiện có và bám theo cấu trúc service/controller/routes/validator của repo.
- [ ] Triển khai đầy đủ scope **AI Character CRUD + prompt template system**; nối route hoặc middleware vào entrypoint thực tế.
...
- [ ] Đối chiếu kết quả với yêu cầu cốt lõi: System prompt builder, admin CRUD.
...
> Added by BMAD Advanced Elicitation on 2026-05-31. This section defines the concrete product output expected from issue #84 / `T-E02`, beyond implementation process notes.
...
- Một hoặc nhiều endpoint dưới `/api/v1/ai/characters` hoạt động với request hợp lệ và trả response chuẩn `{ success, data, meta? }`.
...
- Client gọi `/api/v1/ai/characters`, nhận HTTP 2xx với data đủ field để consumer render/lưu state.
...
- PR description must link issue #84 and mention `T-E02`.
