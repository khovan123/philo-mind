...
- Issue: [#98](https://github.com/khovan123/philo-mind/issues/98)
- State: done
...
- Updated at: 2026-06-04T09:29:33Z
- Closed at: 2026-06-04T09:29:33Z
...
- 2026-05-31: Synced from GitHub issue state. This local file exists so the plan has an auditable log for issue #98 / `T-F06`.
...
| GitHub issue   | #98                                                                                       |
| Task ID        | T-F06                                                                                     |
| Title          | Debate list + detail screens (split FOR/AGAINST view)                                     |
| State          | done                                                                                      |
| Local log path | `issues/by-github-id/#098-T-F06-Debate list + detail screens (split FOR-AGAINST view).md` |
...
- Dependency trước khi nối API thật: `T-F05`.
- User mở màn hình qua route `/debates`; các CTA phải điều hướng tới màn hình chi tiết hoặc bước kế tiếp có data id/session id.
- Nếu backend chưa sẵn sàng, tạo adapter/mock cùng shape với API thật để khi issue dependency merge chỉ thay data source.
- Issue này phải được triển khai trên branch riêng và PR phải link trực tiếp tới issue #98.
- Nếu phát hiện dependency chưa sẵn sàng, PR phải ghi rõ mock/contract tạm và điều kiện để chuyển sang integration thật.

## 5. Hành vi người dùng hoặc API cần đạt

- Người dùng có thể mở màn hình tại `/debates` từ tab/card/link liên quan.
...
- [ ] Argument cards: có bằng chứng kiểm chứng rõ ràng trong PR.
- [ ] vote animation: có bằng chứng kiểm chứng rõ ràng trong PR.
...
> Added by BMAD Advanced Elicitation on 2026-05-31. This section defines the concrete product output expected from issue #98 / `T-F06`, beyond implementation process notes.
...
- Một màn hình/flow tại `/debates/[id]` render được trạng thái loading, empty, error và success.
...
- User thao tác trên `/debates/[id]`, thấy dữ liệu/render đúng, CTA chính chuyển sang bước kế tiếp hoặc cập nhật UI ngay.
...
- PR description must link issue #98 and mention `T-F06`.
